...
local function Update_ZoneText()
	if not xMap or not yMap or not location then return end
...
local function UpdateSettings()
	...
	local holder = ElvUI_MinimapHolder
...
function ML:UpdateSettings()
	...
	elseif init then
		...
		local ElvUI_MinimapHolder = ElvUI_MinimapHolder
		local point, relativeTo, relativePoint = ElvUI_MinimapHolder:GetPoint()
		ElvUI_MinimapHolder:Point(point, relativeTo, relativePoint, 0, 0)

		if E.db.datatexts.minimapPanels then
			ElvUI_MinimapHolder:Height(Minimap:GetHeight() + (LeftMiniPanel and (LeftMiniPanel:GetHeight() + E.Border) or 24) + E.Spacing * 3)
		else
			ElvUI_MinimapHolder:Height(Minimap:GetHeight() + E.Border + E.Spacing * 3)
		end

		if MinimapMover then
			MinimapMover:Size(ElvUI_MinimapHolder:GetSize())
		end
